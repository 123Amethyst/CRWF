sap.ui.define([
	...
	return BaseController.extend('crdt.wf.ui5.controller.Information.FormUpdateVBAAR', {
		//初始化
		onInit: function () {
			this.initLanguage()
		...
		DataLoad: function () {
			...
			if (_backgroundData['HD']['REQID'] != null && _backgroundData['HD']['REQID'] != "") {
				if (_backgroundData['AR'] != null)
				{
					...
					if (_currentUser["RoleId"].length > 0) {
						for (var j = 0; j < _currentUser["RoleId"].length; j++) {
							if ((_currentUser["RoleId"][j] == "Clerk" && _backgroundData['HD']['ACTCD'] != "DRAFT") || _currentUser["RoleId"][j] == "Acct" || _currentUser["RoleId"][j] == "AM" || _currentUser["RoleId"][j] == "FD" || _currentUser["RoleId"][j] == "FM") {
							...
							else if (_backgroundData['ApplicantUserProperty']["RoleId"][j] == "Clerk" && _backgroundData['HD']['ACTCD'] == "DRAFT") {
								this.byId("RiskClass").setVisible(true);
								this.byId("CheckRule").setVisible(true);
								this.byId("VirtualBankAccount").setVisible(true);
								//this.byId("ReconciliationAccount").setVisible(true);
			...
			else
			{
				...
				this.byId("Others").setSelected(false);
				this.byId("Waived").setSelected(false);
				if (_backgroundData['ApplicantUserProperty']["RoleId"].length > 0) {
